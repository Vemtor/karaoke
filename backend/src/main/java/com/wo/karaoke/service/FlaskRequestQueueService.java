...
public class FlaskRequestQueueService {
  ...
  public Map<String, Object> processAudioQueued(MultipartFile file) throws Exception {
    ...
    log.info(
        "Added transcription task to queue for file: {}. Queue size: {}",
        file.getOriginalFilename(),
        taskQueue.size());
    try {
      processingStarted.get();
      log.info(
          "Processing started for file: {}, applying timeout of {} minutes",
          file.getOriginalFilename(),
          PROCESSING_TIMEOUT);
        ...
        () -> {
          ...
          while (!Thread.currentThread().isInterrupted()) {
            try {
              ...
              log.info(
                  "Processing transcription task for file: {}. Remaining tasks: {}",
                  task.file.getOriginalFilename(),
                  taskQueue.size());
  ...
  private record TranscriptionTask(
      MultipartFile file,
